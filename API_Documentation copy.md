...
### GET `/admin/users/search`、`/teacher/users/search`
- **说明**：按关键词搜索用户，支持角色与状态筛选与分页。
- **查询参数**
  | 名称 | 说明 |
  |------|------|
  | `q` | 必填，关键词，匹配 `name/account/email/phone` |
...
- **查询参数**
  | 名称 | 说明 |
  |------|------|
  | `q` _(可选)_ | 关键词（模糊匹配 `labname`/`labnumber`/`teacher`/`material`） |
  | `labnumber` _(可选)_ | 实验室编号 |
  | `teacher` _(可选)_ | 负责人 |
...
### DELETE `/courses`
- **说明**：根据 `lab_id` + `date` + `time_slot` 条件删除课程。
- **查询参数**：`lab_id` (int)、`date` (`YYYY-MM-DD`)、`time_slot`。
    ...
    "data": [
      {
        ...
        "labname": "工业设计实验室",
        "participants": [
          { "user_id": 12, "account": "student01", "name": "张三", "role": "student" }
        ],
        "photo_urls": [],
