...
export const getReservations = async (params?: ReservationQuery) => {
  const { data } = await http.get<ReservationListResponse>('/reservations', { params })
  return data
}

export const getReservationDetail = async (id: number) => {
  const { data } = await http.get<ReservationDetailResponse>(`/reservations/${id}`)
  return data
}

export const createReservation = async (payload: CreateReservationPayload) => {
  const { data } = await http.post<CreateReservationResponse>('/reservations', payload)
  return data
}

export const cancelReservation = async (id: number) => {
  const { data } = await http.delete<CancelReservationResponse>(`/reservations/${id}`)
  return data
}
